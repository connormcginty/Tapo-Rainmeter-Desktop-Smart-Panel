...
async def update_status(device_name):
    config = load_config()
    ...
    write_status(device_name, status)
    print(f"{device_name} is {status}")


def main():
    if len(sys.argv) != 2:
        print("Usage: python tapo_status.py <device_name>")
        print("Example: python tapo_status.py officefan")
        raise SystemExit(1)

    device_name = sys.argv[1].strip().lower()
    asyncio.run(update_status(device_name))
